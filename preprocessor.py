...
for vidno in range(len(train_path)):
    ...
    for i in range(len(cutpoint_rising)):
        if (librosa.frames_to_time(cutpoint_falling[i],sr=sr)-librosa.frames_to_time(cutpoint_rising[i],sr=sr))>=0.3:
            ...
            print(("ffmpeg -y -ss " + str(librosa.frames_to_time(cutpoint_rising[i],sr=sr)) + " -t " + str(librosa.frames_to_time(cutpoint_falling[i],sr=sr)-librosa.frames_to_time(cutpoint_rising[i],sr=sr)) + " -i " +str(train_path[vidno])+" ./dataset/"+str(train_path[vidno].split('/')[-1].split(".")[0])+'_'+str(i)+".wav"))
            os.system(("ffmpeg -y -ss " + str(librosa.frames_to_time(cutpoint_rising[i],sr=sr)) + " -t " + str(librosa.frames_to_time(cutpoint_falling[i],sr=sr)-librosa.frames_to_time(cutpoint_rising[i],sr=sr)) + " -i " +str(train_path[vidno])+" ./dataset/"+str(train_path[vidno].split('/')[-1].split(".")[0])+'_'+str(i)+".wav"))